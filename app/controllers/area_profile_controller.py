...
from app.services.area_profile import area_profile_service
...
router = APIRouter()
TAG = 'Map Area Profile'
LOGIN = Depends(JWTBearer())
DB = Depends(get_db)

@router.get("/area_profile/list/{id}", dependencies=[LOGIN], tags=[TAG])
def get_all_profile(request: Request, id: int, session: Session = DB):
    user_id = system_service.get_current_id(request)
    response = area_profile_service.get_all_by_user_id(session, id, user_id)
    system_log_service.system_log_create(session, request, OverideSystemLogSchema(
        action=LOG_ACTION.LIST,
        type=LOG_TYPE.LOG_AREA_PROFILE,
        description='Danh sách hồ sơ của khu vực được đánh dấu',
    ))
    return response

@router.post("/area_profile/create", dependencies=[LOGIN], tags=[TAG])
async def area_profile_create(request: Request, session: Session = DB, body: AreaProfileCreateSchema = Body(...)):
    user_id = system_service.get_current_id(request)
    response = area_profile_service.create_area_profile(session, user_id, body)
    system_log_service.system_log_create(session, request, OverideSystemLogSchema(
...
@router.put("/area_profile/update/{id}", dependencies=[LOGIN], tags=[TAG])
async def area_profile_update(request: Request, id: int, session: Session = DB, body: AreaProfileUpdateSchema = Body(...)):
    user_id = system_service.get_current_id(request)
    response = area_profile_service.update_area_profile(id, session, body)
    system_log_service.system_log_create(session, request, OverideSystemLogSchema(
        action=LOG_ACTION.CREATE,
        type=LOG_TYPE.LOG_AREA_PROFILE,
...
@router.delete("/area_profile/delete/{id}", dependencies=[LOGIN], tags=[TAG])
async def area_profile_delete(request: Request, id: int, session: Session = DB):
    user_id = system_service.get_current_id(request)
    response = area_profile_service.delete_area_profile(user_id, id, session)
    system_log_service.system_log_create(session, request, OverideSystemLogSchema(
